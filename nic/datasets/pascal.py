...
def makejson():
    tokenpath = './data/pascal/sentence/Caption'
    ann_list = dd(list)
    imageinfo_list = {}
    buildvocab_dict = {'annotations': []}
    ann_out = './data/pascal/sentence/pascal_ann.json'
    imageinfo_out = './data/pascal/sentence/pascal_imageinfo.json'
    id = 0
...
def generate_test_entries(annFile= "./data/pascal/sentence/pascal_ann.json" , root="./data/pascal/",
                          new_test_filename="captions_pascal_test.json",
                          imageinfo_filename = "./data/pascal/sentence/pascal_imageinfo.json"):
    """
    ...
    test_origin = root + "sentence/pascal_test.txt"
...
def get_data_loader(mode, transform, vocab, batch_size=4, shuffle=True, num_workers=0, data_dir = 1):
    ...
    root = "./data/pascal/dataset/"
    annFile = "./data/pascal/captions_pascal_test.json"

    dataset = PascalDataset(root=root,
					  annFile=annFile,
					  vocab=vocab,
					  transform=transform)
    print(len(dataset))
...
def main(args):
    ...
    dataloader = get_data_loader(None, transform, vocab)
